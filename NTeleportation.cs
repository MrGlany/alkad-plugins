...
{
    [Info("NTeleportation", "MrCodder", "1.9.6")]
    ...
    class NTeleportation : RustPlugin
    {
        ...
        private readonly List<(Vector3 position, float sqrDistance)> safeZones = new();
        private readonly List<ApartmentRoom> _rooms = new();
        private readonly List<ApartmentBuilding> _apartmentBuildings = new();

        private ApartmentBuilding GetApartmentBuilding()
        {
            if (_apartmentBuildings.Count == 0)
            {
                foreach (var triggerSafeZone in TriggerSafeZone.allSafeZones)
                {
                    if (triggerSafeZone.Apartment == null) continue;
                    _apartmentBuildings.Add(triggerSafeZone.Apartment);
                }
            }
            return _apartmentBuildings.Count > 0 ? _apartmentBuildings[0] : null;
        }
        ...
        private bool IsInsideApartmentRoom(BasePlayer player)
        {
            ...
            {
                if (IsAlly(player, owner, config.Home.UseTeams, config.Home.UseClans, config.Home.UseFriends))
                {
                    return true;
                }
            }
        ...
        private bool IsInApartmentComplex(Vector3 v) => apartments != null && apartments.IsInBounds(v);
        ...
        public void DelayedTeleportHome(BasePlayer player)
        {
            ...
            ServerMgr.Instance.Invoke(() =>
            {
                ...
                if (IsInsideApartmentRoom(player))
                {
                    delayedTeleports.Remove(userid);
                    return;
                }
        ...
        public class HomesSettings
        {
            ...
            [JsonProperty(PropertyName = "Allow Apartment Complex")]
            public bool ApartmentComplex = true;
        ...
        exit:
            ...
            if (text == "Apartment Complex")
            {
                from += rot * new Vector3(-31f, 0f, 22f);
                x = y = z = x + 15f;
            }
        final:
        ...
        private void TeleportBack(BasePlayer player, AdminData adminData, int countdown)
        {
            ...
            teleportTimer.action = () =>
            {
                ...
                if (!CanBypassRestrictions(player.UserIDString))
                {
                    err = CheckPlayer(player, location, GetTownUsableOutOfBuildingBlocked(player, config.Home.UsableOutOfBuildingBlocked), CanCraftHome(player), true, config.Home.RemoveHostility, "home", CanCaveHome(player), true, config.Home.DeepSea, "TPDeepSeaFrom");
                    if (err != null)
                    {
        ...
        private bool GetTownUsableOutOfBuildingBlocked(BasePlayer player, bool def)
        {
            var a = player.transform.position;
            foreach (var x in config.DynamicCommands)
            {
                foreach (var b in x.Value.Locations)
                {
                    if ((a - b).sqrMagnitude < 50f)
                    {
                        return x.Value.UsableOutOfBuildingBlocked;
                    }
                }
            }
            return def;
        ...
        private void cmdChatHomeTP(BasePlayer player, string command, string[] args)
        {
            ...
            if (!CanBypassRestrictions(player.UserIDString))
            {
                ...
                err = CheckPlayer(player, Vector3.zero, GetTownUsableOutOfBuildingBlocked(player, config.Home.UsableOutOfBuildingBlocked), CanCraftHome(player), true, config.Home.RemoveHostility, "home", CanCaveHome(player), true, config.Home.DeepSea, "TPDeepSeaFrom");
                if (err != null)
                {
                    PrintMsgL(player, err);
                    return;
                ...
                //player.ChatMessage($"Cooldown: {cooldown}, Elapsed: {timestamp - homeData.Teleports.Timestamp}, Remain: {remain}");
                ...
                if (cooldown > 0 && timestamp - homeData.Teleports.Timestamp < cooldown)
                {
                    ...
                    if (!string.IsNullOrEmpty(config.Settings.BypassCMD) && !paidmoney)
                    {
                        ...
                        else if (UseEconomy() && config.Home.Bypass > 0)
                        {
            ...
            teleportTimer.action = () =>
            {
                ...
                Teleport(player, position, args[0], 0uL, town: false, allowTPB: config.Home.AllowTPB, removeHostility: config.Home.RemoveHostility, deepsea: config.Home.DeepSea, build: GetTownUsableOutOfBuildingBlocked(player, config.Home.UsableOutOfBuildingBlocked), craft: CanCraftHome(player), cave: CanCaveHome(player));
        ...
        private void CommandTeleportRequest(IPlayer user, string command, string[] args)
        {
            ...
            if (!CanBypassRestrictions(player.UserIDString))
            {
                ...
                err = CheckPlayer(player, Vector3.zero, GetTownUsableOutOfBuildingBlocked(player, config.TPR.UsableOutOfBuildingBlocked), CanCraftTPR(player), true, config.TPR.RemoveHostility, "tpr", CanCaveTPR(player), true, config.TPR.DeepSea, "TPDeepSeaFrom");
                if (err != null)
                {
                    PrintMsgL(player, err);
                    return;
                }
                var isAlly = IsAlly(target, player, config.TPT.UseTeams, config.TPT.UseClans, config.TPT.UseFriends);
        ...
        private void CommandTeleportAccept(IPlayer user, string command, string[] args)
        {
            ...
            if (!CanBypassRestrictions(player.UserIDString))
            {
                ...
                err = CheckPlayer(originPlayer, player.transform.position, GetTownUsableOutOfBuildingBlocked(originPlayer, config.TPR.UsableOutOfBuildingBlocked), CanCraftTPR(originPlayer), true, config.TPR.RemoveHostility, "tpa", CanCaveTPR(originPlayer), true, config.TPR.DeepSea, "TPDeepSeaFrom");
                if (err != null)
                {
                ...
                bool cupAllyAllowOnBuildingBlocked = config.TPR.CupNonAllyAllowOnBuildingBlocked || config.TPR.CupAllyAllowOnBuildingBlocked && IsAlly(originPlayer, player, config.TPT.UseTeams, config.TPT.UseClans, config.TPT.UseFriends);
            ...
            teleportTimer.action = () =>
            {
                ...
                if (!CanBypassRestrictions(player.UserIDString))
                {
                    ...
                    err = CheckPlayer(originPlayer, player.transform.position, GetTownUsableOutOfBuildingBlocked(originPlayer, config.TPR.UsableOutOfBuildingBlocked), CanCraftTPR(originPlayer), true, config.TPR.RemoveHostility, "tpa", CanCaveTPR(originPlayer), true, config.TPR.DeepSea, "TPDeepSeaTo") ??
                          CheckPlayer(player, Vector3.zero, false, CanCraftTPR(player), true, config.TPR.RemoveHostility, "tpa", CanCaveTPR(player), IsAlly(originPlayer, player, config.Home.UseTeams, config.Home.UseClans, config.Home.UseFriends), config.TPR.DeepSea, "TPDeepSeaFrom");
                    if (err != null)
                    {
                ...
                Teleport(originPlayer, player.transform.position, "", player.userID, town: false, allowTPB: config.TPR.AllowTPB, removeHostility: config.TPR.RemoveHostility, deepsea: config.TPR.DeepSea, build: GetTownUsableOutOfBuildingBlocked(originPlayer, config.TPR.UsableOutOfBuildingBlocked), craft: CanCraftTPR(player), CanCaveTPR(player));
        ...
        private void CommandSphereMonuments(IPlayer user, string command, string[] args)
        {
            ...
            if (ConVar.Server.hostname.Contains("Test Server")) Puts("Sent {0} text, {1} sphere and {2} line ddraw commands", text, sphere, line);
        }
        ...
        private bool Teleport(BasePlayer player, Vector3 oldPosition, Vector3 newPosition, string home, ulong uid, bool town, bool allowTPB, bool removeHostility, bool deepsea, bool build, bool craft, bool cave, bool death)
        {
            ...
            if (player.currentGesture != null) player.Server_CancelGesture();
            ...
            if (player.IsConnected)
            {
                if (!player.limitNetworking && !player.isInvisible)
                {
                    try { player.UpdateNetworkGroup(); } catch { }
        ...
        private string NearMonument(Vector3 target, bool check, string mode)
        {
            ...
            foreach (var mi in monuments)
            {
                ...
                if (isInBounds)
                {
                    if (config.Home.ApartmentComplex && mi.name == "Apartment Complex")
                    {
                        return null;
                    }
        ...
        private string CheckPlayer(BasePlayer player, Vector3 zonePos, bool build, bool craft, bool origin, bool removeHostility, string mode, bool allowcave, bool ff, bool deepsea, string deepSeaKey)
        {
            ...
            {
                string monname = !config.Settings.Interrupt.Safe && player.InSafeZone() && !player.InSafeCombatZone() ? null : NearMonument(player.transform.position, false, mode);

                if (!string.IsNullOrEmpty(monname))
                {
                    if (config.Home.ApartmentComplex && monname == "Apartment Complex")
                    {
                        return null;
                    }
            ...
            {
                if (!config.Home.ApartmentComplex || !IsInApartmentComplex(player.transform.position))
                {
                    return "TPSafeZone";
                }
            }
        ...
        {
            if (Physics.Raycast(a + new Vector3(0f, 3f, 0f), Vector3.down, out var hit, 5f, Layers.Mask.Construction, QueryTriggerInteraction.Ignore))
            {
                var e = hit.GetEntity();
        ...
        private string CheckFoundation(BasePlayer player, ulong userid, Vector3 position, string mode)
        {
            ...
            if (!config.Home.ForceOnTopOfFoundation || permission.UserHasPermission(userid.ToString(), PermFoundationCheck) || IsInApartmentComplex(position))
            {
                return null;
            }
            ...
            {
                bool cupAllyOrNon = config.Home.CupNonAllyAllowOnBuildingBlocked || config.Home.CupAllyAllowOnBuildingBlocked && IsAlly(player, block.OwnerID, config.Home.UseTeams, config.Home.UseClans, config.Home.UseFriends);
            ...
            if (player.userID != userid || !config.Home.CheckFoundationForOwner || entity is Tugboat || IsAlly(player, entity.OwnerID, config.Home.UseTeams, config.Home.UseClans, config.Home.UseFriends))
            {
                return null;
            }
        ...
        private bool IsAlly(BasePlayer a, BasePlayer b, bool teams, bool clans, bool friends)
        {
            if (clans && a.serverClan != null && a.clanId != 0 && a.clanId == b.clanId) return true;
            return IsAlly(a.userID, b.userID, teams, clans, friends);
        }

        private bool IsAlly(BasePlayer a, ulong b, bool teams, bool clans, bool friends)
        {
            if (clans && a.serverClan != null)
            {
            ...
            return IsAlly(a.userID, b, teams, clans, friends);
        }

        private bool IsAlly(ulong a, ulong b, bool useTeams, bool useClans, bool useFriends)
        {
            if (a == b)
            {
                return true;
            }
            if (useTeams && RelationshipManager.ServerInstance.playerToTeam.TryGetValue(a, out var team) && team.members.Contains(b))
            {
                return true;
            }
            if (useClans && Clans != null && Clans.IsLoaded && Convert.ToBoolean(Clans?.Call("IsClanMember", a.ToString(), b.ToString())))
            {
                return true;
            }
            if (useFriends && Friends != null && Friends.IsLoaded && Convert.ToBoolean(Friends?.Call("AreFriends", a.ToString(), b.ToString())))
            {
                return true;
            }
            return false;
        ...
        private void SendDiscordMessage(BasePlayer player, BasePlayer target)
        {
            ...
            if (player.userID == target.userID || IsAlly(player, target, true, true, true))
            {
                return;
            }
